#ifndef WTLEXICALCAST_H
#define WTLEXICALCAST_H
...
struct LexicalCastFailed : WTException
{
	LexicalCastFailed(std::string const& reason): WTException(reason, false) { }
};
...
namespace Internal {
	...
	void CheckStreamStatus(STREAM const& stream, bool checkEOF)
	{
		...
		if (stream.fail())
		{
			...
			throw LexicalCastFailed("Failed to convert value");
		}
		if (checkEOF && !stream.eof())
		{
			// E.g. you tried converting "62 61" or "61p" to an int...
			throw LexicalCastFailed("Failed to convert value as too much data was supplied");
		}
	}
	...
	struct ToStringCaster<std::wstring>
	{
		...
		{
			return wtStringUtils::WStringToString(val);
		}
	};
	...
	struct ToWStringCaster<std::string>
	{
		...
		{
			return wtStringUtils::StringToWString(val);
		}
	};
